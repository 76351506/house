...
class Housing extends Controller {
    ...
    async housing() {
        ...
        const data = await this.app.mysql.query(`SELECT * FROM renthouses WHERE quyu LIKE '%${area}%' AND name LIKE '%${name}%' limit ${(pageCount-1)*pageSize},${pageSize}`);
        ctx.body = {
            code: 200,
    ...
    async setMarketBroker() {
        ...
        const data = await this.app.mysql.query(`SELECT * FROM brokers WHERE state LIKE '%在职%' order by new_house asc`)
        ctx.body = {
            code: 200,
            data
        }
